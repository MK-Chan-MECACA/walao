# WALAO — product specification / 产品规格

> Full bilingual product specification. Version 0.4 consolidated, updated 2026-07-26.
> Split out of the repository README so the README can stay short.
...
| `contacts` | `session_id`, `jid`, `name` | 仅存显示名，用于还原 @提及；随 Session 一并删除 |
...
| `contacts` | `session_id`, `jid`, `name` | Display names only, so an @mention reads as a person; deleted with the Session |
